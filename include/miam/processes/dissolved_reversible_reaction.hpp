...
namespace miam
{
    namespace process
    {
        ...
        struct DissolvedReversibleReaction
        {
            ...
            /// @param phase_prefixes Map of phase names to sets of state variable prefixes (prefix does not include phase or species names)
            /// @param state_parameter_indices Map of state parameter names to their corresponding indices in the state parameter vector
            /// @param state_variable_indices Map of state variable names to their corresponding indices in the state variable vector
            ...
            ) const
            {
                ...
                return DenseMatrixPolicy::Function(
                    [this, variable_indices, forward_index, reverse_index](auto&& state_parameters, auto&& state_variables, auto&& forcing_terms)
                    {
                        ...
                        for (std::size_t i_phase = 0; i_phase < variable_indices.number_of_phase_instances_; ++i_phase)
                        {
                            ...
                            state_parameters.ForEachRow([&](const double& forward_rate_constant, const double& reverse_rate_constant, const double& solvent, double& forward_rate, double& reverse_rate)
                            {
                            ...
                            {
                                state_variables.ForEachRow([&](const double& reactant, double& forward_rate)
                                {
                                    forward_rate *= reactant;
                                }, state_variables.GetConstColumnView(variable_indices.reactant_indices_[i_phase][r]), forward_rate);
                            }
                            for (std::size_t p = 0; p < products_.size(); ++p)
                            {
                                state_variables.ForEachRow([&](const double& product, double& reverse_rate)
                                {
                                    reverse_rate *= product;
                                }, state_variables.GetConstColumnView(variable_indices.product_indices_[i_phase][p]), reverse_rate);
                            }

                            // Apply the reaction rates to the forcing terms for reactants and products
                            for (std::size_t r = 0; r < reactants_.size(); ++r)
                            {
                                state_variables.ForEachRow([&](const double& forward_rate, const double& reverse_rate, double& forcing)
                                {
                                    forcing -= forward_rate;
                                    forcing += reverse_rate;
                                }, forward_rate, reverse_rate, forcing_terms.GetColumnView(variable_indices.reactant_indices_[i_phase][r]));
                            }
                            for (std::size_t p = 0; p < products_.size(); ++p)
                            {
                                state_variables.ForEachRow([&](const double& forward_rate, const double& reverse_rate, double& forcing)
                                {
                                    forcing += forward_rate;
                                    forcing -= reverse_rate;
                                }, forward_rate, reverse_rate, forcing_terms.GetColumnView(variable_indices.product_indices_[i_phase][p]));
                            }
                        }
                    }, dummy_state_parameters, dummy_state_variables, dummy_state_variables);
            }
            ...
            ) const
            {
                ...
                return SparseMatrixPolicy::Function(
                    ...
                    (auto&& state_parameters, auto&& state_variables, auto&& jacobian_values)
                    {
                        ...
                        for (std::size_t i_phase = 0; i_phase < variable_indices.number_of_phase_instances_; ++i_phase)
                        {
                            // Calculate the partial derivatives of the forward and reverse rates with respect to the independent variable
                            ...
                            for (std::size_t i_ind = 0; i_ind < reactants_.size(); ++i_ind)
                            {
                                ...
                                jacobian_values.ForEachBlock([&](const double& forward_rate_constant, const double& solvent, double& partial)
                                {
                                ...
                                for (std::size_t r = 0; r < reactants_.size(); ++r)
                                {
                                    if (r==i_ind) continue; // Skip the variable we're taking the derivative with respect to
                                    jacobian_values.ForEachBlock([&](const double& reactant, double& partial)
                                    {
                                        partial *= reactant;
                                    }, state_variables.GetConstColumnView(variable_indices.reactant_indices_[i_phase][r]), d_forward_rate_d_ind);
                                }
                                // apply partial to dependent reactants
                                for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                                {
                                    jacobian_values.ForEachBlock([&](const double& partial, double& jacobian)
                                    {
                                        jacobian -= partial;
                                    }, d_forward_rate_d_ind, jacobian_values.GetBlockView(*jac_id++));
                                }
                            ...
                            {
                                // Start the rate calculation with the rate constant and solvent
                                jacobian_values.ForEachBlock([&](const double& reverse_rate_constant, const double& solvent, double& partial)
                                {
                                ...
                                for (std::size_t p = 0; p < products_.size(); ++p)
                                {
                                    if (p==i_ind) continue; // Skip the variable we're taking the derivative with respect to
                                    jacobian_values.ForEachBlock([&](const double& product, double& partial)
                                    {
                                        partial *= product;
                                    }, state_variables.GetConstColumnView(variable_indices.product_indices_[i_phase][p]), d_reverse_rate_d_ind);
                                }
                                // apply partial to dependent reactants
                                for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                                {
                                    ...
                                    {
                                        jacobian += partial;
                                    }, d_reverse_rate_d_ind, jacobian_values.GetBlockView(*jac_id++));
                                }
                                // apply partial to dependent products
                                for (std::size_t i_dep = 0; i_dep < products_.size(); ++i_dep)
                                {
                            ...
                            jacobian_values.ForEachBlock([&](const double& forward_rate_constant, const double& reverse_rate_constant, const double& solvent, double& forward_partial, double& reverse_partial)
                            {
                                forward_partial = forward_rate_constant * (1 - static_cast<int>(reactants_.size())) / std::pow(solvent, reactants_.size());
                                reverse_partial = reverse_rate_constant * (1 - static_cast<int>(products_.size())) / std::pow(solvent, products_.size());
                            }, state_parameters.GetConstColumnView(forward_index),
                               state_parameters.GetConstColumnView(reverse_index),
                               state_variables.GetConstColumnView(variable_indices.solvent_indices_[i_phase]),
                               d_forward_rate_d_ind, d_reverse_rate_d_ind);
                            // add contributions to the partial from the reactants/products
                            for (std::size_t r = 0; r < reactants_.size(); ++r)
                            {
                                jacobian_values.ForEachBlock([&](const double& reactant, double& forward_partial)
                                {
                                    forward_partial *= reactant;
                                }, state_variables.GetConstColumnView(variable_indices.reactant_indices_[i_phase][r]), d_forward_rate_d_ind);
                            }
                            for (std::size_t p = 0; p < products_.size(); ++p)
                            {
                                jacobian_values.ForEachBlock([&](const double& product, double& reverse_partial)
                                {
                                    reverse_partial *= product;
                                }, state_variables.GetConstColumnView(variable_indices.product_indices_[i_phase][p]), d_reverse_rate_d_ind);
                            }
                            // apply partials to dependent reactants
                            for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                            {
                                jacobian_values.ForEachBlock([&](const double& forward_partial, const double& reverse_partial, double& jacobian)
                                {
                                    jacobian -= forward_partial;
                                    jacobian += reverse_partial;
                                }, d_forward_rate_d_ind, d_reverse_rate_d_ind, jacobian_values.GetBlockView(*jac_id++));
                            }
                    ...
                    }, dummy_state_parameters, dummy_state_variables, jacobian);
            }
                                 
        private:

            /// @brief Helper struct for keeping track of state varible indices for reactants, products, and solvent across multiple phase instances (e.g. grid cells)
            struct StateVariableIndices
            {
                std::size_t number_of_phase_instances_; ///< Number of instances of the phase in the system (e.g. number of grid cells containing this phase)
                micm::Matrix<std::size_t> reactant_indices_; ///< Matrix of state variable indices for reactants (num_reactants x num_prefixes)
                micm::Matrix<std::size_t> product_indices_;   ///< Matrix of state variable indices for products (num_products x num_prefixes)
            ...
            {
                micm::Matrix<std::size_t> indices_; // Index in sparse matrix for each dependent/independent pair (num_pairs x num_prefixes)
            };
            ...
            ) const
            {
                ...
                if (state_parameter_indices.find(forward_param) == state_parameter_indices.end())
                {
                    throw std::runtime_error("Internal Error: GetParameterIndices: Forward rate constant parameter " + forward_param + " not found in state_parameter_indices");
                }
                if (state_parameter_indices.find(reverse_param) == state_parameter_indices.end())
                {
                    throw std::runtime_error("Internal Error: GetParameterIndices: Reverse rate constant parameter " + reverse_param + " not found in state_parameter_indices");
                }
            ...
            ) const
            {
                ...
                if (phase_it == phase_prefixes.end())
                {
                    throw std::runtime_error("Internal Error: GetStateVariableIndices: Phase " + phase_.name_ + " not found in phase_prefixes");
                }
                const auto& prefixes = phase_it->second;
                ...
                for (const auto& prefix : prefixes)
                {
                    for (std::size_t i_reactant = 0; i_reactant < reactants_.size(); ++i_reactant)
                    {
                        ...
                        {
                            throw std::runtime_error("Internal Error: GetStateVariableIndices: Reactant variable " + reactant_var + " not found in state_variable_indices");
                        }
                    ...
                    for (std::size_t i_product = 0; i_product < products_.size(); ++i_product)
                    {
                        ...
                        {
                            throw std::runtime_error("Internal Error: GetStateVariableIndices: Product variable " + product_var + " not found in state_variable_indices");
                        }
                    ...
                    {
                        throw std::runtime_error("Internal Error: GetStateVariableIndices: Solvent variable " + solvent_var + " not found in state_variable_indices");
                    }
            ...
            /// @param variable_indices StateVariableIndices struct containing matrices of indices for reactants, products, and solvent
            /// @param jacobian Sparse matrix policy object for the Jacobian structure
            /// @return JacobianIndices struct containing a matrix of sparse matrix indices for each dependent/independent pair of species
            ...
            ) const
            {
                ...
                std::size_t num_pairs = (reactants_.size() + products_.size()) * (reactants_.size() + products_.size() + 1); // +1 for solvent
                ...
                for (std::size_t i_phase = 0; i_phase < variable_indices.number_of_phase_instances_; ++i_phase)
                {
                    ...
                    for (std::size_t i_ind = 0; i_ind < reactants_.size(); ++i_ind)
                    {
                        ...
                        for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                        {
                            jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.reactant_indices_[i_phase][i_dep], variable_indices.reactant_indices_[i_phase][i_ind]);
                        }
                        // ... and dependent products
                        for (std::size_t i_dep = 0; i_dep < products_.size(); ++i_dep)
                        {
                            jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.product_indices_[i_phase][i_dep], variable_indices.reactant_indices_[i_phase][i_ind]);
                        }
                    }
                    ...
                    for (std::size_t i_ind = 0; i_ind < products_.size(); ++i_ind)
                    {
                        ...
                        for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                        {
                            jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.reactant_indices_[i_phase][i_dep], variable_indices.product_indices_[i_phase][i_ind]);
                        }
                        // ... and dependent products
                        for (std::size_t i_dep = 0; i_dep < products_.size(); ++i_dep)
                        {
                            jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.product_indices_[i_phase][i_dep], variable_indices.product_indices_[i_phase][i_ind]);
                        }
                    }
                    ...
                    for (std::size_t i_dep = 0; i_dep < reactants_.size(); ++i_dep)
                    {
                        jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.reactant_indices_[i_phase][i_dep], variable_indices.solvent_indices_[i_phase]);
                    }
                    // ... and dependent products
                    for (std::size_t i_dep = 0; i_dep < products_.size(); ++i_dep)
                    {
                        jacobian_indices.indices_[i_phase][pair_index++] = jacobian.VectorIndex(0, variable_indices.product_indices_[i_phase][i_dep], variable_indices.solvent_indices_[i_phase]);
                    }                
                }
                return jacobian_indices;
            }       
