...
public:
  ...
  double GetEffectiveRadius(const StateType& state, std::size_t cell) const 
  {
    ...
    return CalculateEffectiveRadius(total_mass, number_concentration, 
                                    density, geometric_standard_deviation_);
  }
  ...
  { 
    ...
    state_idx_.number_id = state.variable_map_.at(JoinStrings(
      { name_, AerosolScheme::AEROSOL_MOMENTS_[0]}));

    // Find density index
    state_idx_.density_id = state.variable_map_.at(JoinStrings(
      { name_, AerosolScheme::AEROSOL_MOMENTS_[1]}));
