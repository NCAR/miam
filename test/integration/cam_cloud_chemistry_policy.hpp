...
//
// Templated integration-test policies for the CAM cloud aqueous-phase
// sulfate-chemistry stack in MIAM. Each policy function takes a partially
// configured MICM solver `BuilderPolicy` (system, external model, etc. are
// applied inside), so the same test logic can be driven with any solver
// matrix backing (standard/vector CPU matrices, Kokkos, etc.).
//
// The Jacobian-verification helpers below (`VerifyProcessJacobian`,
// `VerifyConstraintJacobian`) use fixed CPU dense/sparse matrix types since
// they test the analytical Model Jacobian against a finite-difference
// reference; they are backend-independent w.r.t. the solver under test.
//
// UNIT CONVENTIONS:
...
namespace miam_test_cam_cloud_chemistry
{
  ...
  // CPU dense/sparse types used only by the FD-Jacobian verification helpers
  ...
  inline constexpr double c_H2O_M = 55.51;  // mol/L (molar conc. of pure liquid water, 1000 g/L / 18.015 g/mol)

  // Realistic cloud liquid water content
  inline constexpr double C_H2O = 0.017;                                         // mol/m³ air (cloud LWC ~ 0.3 g m⁻³)
  ...
  // ════════════════════════════════════════════════════════════════════════
  // Templated policy functions — one per Step in the CAM cloud chemistry stack
  // ════════════════════════════════════════════════════════════════════════
  ...
  void Step1c_KwNaiveIC(BuilderPolicy builder)
{
  ...
  // Tighter constraint-initialization tolerances for this test are expected to
  // be pre-configured on `builder` by the caller (Kw residuals are tiny).
  ...
  void Step4b_NaiveInitialConditions(BuilderPolicy builder)
{
  ...
  std::cout << "\n--- Step 4b: Naive ICs (all aqueous zero) ---" << std::endl;
  state.PrintHeader();
  state.PrintState(0);
