...
void analytical_solution(double A0, double k_f, double k_r, double t, double& A_t, double& B_t)
{
  double K_eq = k_f / k_r;
...
TEST(DissolvedReversibleReactionIntegration, SolventAsProduct)
{
  ...
  EXPECT_NEAR(A_final, A0, tolerance) << "System should be near equilibrium with A ≈ A0";
}
