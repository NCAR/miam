...
namespace
{
  ...
  const std::vector<double> SOLVENT_SWEEP = { 0.017, 1e-3, 1e-6, 1e-9, 1e-12, 0.0 };

  // Extended sweep without the hard-zero for tests that can't handle exactly 0
  const std::vector<double> SOLVENT_SWEEP_NONZERO = { 0.017, 1e-3, 1e-6, 1e-9, 1e-12, 1e-15 };
  ...
  AccumulatedStats IntegrateWithStats(SolverT& solver, StateT& state, double target_time, double dt0)
  {
  ...
  {
    std::cout << "\n┌─── " << test_name << " ───" << std::endl;
    std::cout << std::setw(14) << "H2O [mol/m³]" << std::setw(12) << "converged" << std::setw(10) << "steps" << std::setw(10)
              << "rejected" << std::setw(10) << "f_calls" << std::setw(14) << "failure_mode" << std::endl;
  ...
  {
    std::cout << std::scientific << std::setprecision(1) << std::setw(14) << h2o << std::setw(12)
              << (s.converged ? "YES" : "NO") << std::dec << std::setw(10) << s.total_steps << std::setw(10)
              << s.total_rejected << std::setw(10) << s.total_function_calls << std::setw(14)
              << (s.converged ? "-" : SolverStateToString(s.last_failure_state)) << std::endl;
  }
  ...
  struct CamCloudSpecies
  {
    ...
    Species h2o{ "H2O", { { "molecular weight [kg mol-1]", 0.018 }, { "density [kg m-3]", 1000.0 } } };
  ...
  auto BuildCamCloudModel(CamCloudSpecies& sp)
  {
    ...
    auto eq_kw = DissolvedEquilibriumConstraintBuilder()
                     ...
                     .SetReactants({ sp.h2o })
                     .SetProducts({ sp.hp, sp.ohm })
                     .SetAlgebraicSpecies(sp.ohm)
                     .SetSolvent(sp.h2o)
    ...
    auto eq_ka1 = DissolvedEquilibriumConstraintBuilder()
                      .SetPhase(sp.aqueous_phase)
                      .SetReactants({ sp.so2_aq })
                      .SetProducts({ sp.hso3m, sp.hp })
                      .SetAlgebraicSpecies(sp.hso3m)
                      .SetSolvent(sp.h2o)
    ...
    auto eq_ka2 = DissolvedEquilibriumConstraintBuilder()
                      .SetPhase(sp.aqueous_phase)
                      .SetReactants({ sp.hso3m })
                      .SetProducts({ sp.so3mm, sp.hp })
                      .SetAlgebraicSpecies(sp.so3mm)
                      .SetSolvent(sp.h2o)
...
TEST(SolventRobustness, A4_HenryLawConstraint_SolventSweep)
{
  ...
  for (double h2o_level : SOLVENT_SWEEP)
  {
    ...
    if (stats.converged)
    {
      ...
      if (h2o_level < 1e-9)
      {
        EXPECT_NEAR(g_f, 3.01e-8, 1e-10 * 3.01e-8) << "SO2 should be entirely in gas phase at H2O=" << h2o_level;
      }
    }
  }
  std::cout << std::endl;
...
TEST(SolventRobustness, A5_HLC_Plus_Dissociation_SolventSweep)
{
  ...
  for (double h2o_level : SOLVENT_SWEEP)
  {
    ...
    {
      EXPECT_FALSE(HasNanOrInf(state)) << "NaN/Inf at H2O=" << h2o_level;
      double total = state.variables_[0][i_g] + state.variables_[0][i_aq] + state.variables_[0][i_hso3m];
...
TEST(SolventRobustness, B1_FullMechanism_StaticSweep)
{
  ...
  std::cout << std::setw(14) << "H2O [mol/m³]" << std::setw(12) << "converged" << std::setw(10) << "steps" << std::setw(10)
            << "rejected" << std::setw(10) << "f_calls" << std::setw(12) << "SO4 [mol/m³]" << std::setw(14) << "failure_mode"
            << std::endl;
  ...
  for (double h2o_level : sweep)
  {
    ...
    std::cout << std::scientific << std::setprecision(1) << std::setw(14) << h2o_level << std::setw(12)
              << (stats.converged ? "YES" : "NO") << std::dec << std::setw(10) << stats.total_steps << std::setw(10)
              << stats.total_rejected << std::setw(10) << stats.total_function_calls << std::scientific
              << std::setprecision(3) << std::setw(12) << so4_f << std::setw(14)
              << (stats.converged ? "-" : SolverStateToString(stats.last_failure_state)) << std::endl;
    ...
    if (stats.converged)
    {
      ...
      // S conservation
      double total_S = state.variables_[0][i_so2_g] + state.variables_[0][i_so2_aq] + state.variables_[0][i_hso3m] +
                       state.variables_[0][i_so3mm] + state.variables_[0][i_so4mm] + state.variables_[0][i_so2oohm];
      EXPECT_NEAR(total_S, 1.0 + 3.01e-8, 1e-4 * total_S) << "S budget violated at H2O=" << h2o_level;
    }
  }
  std::cout << std::endl;
}
...
TEST(SolventRobustness, B2_DynamicEvaporation)
{
  ...
  std::cout << std::setw(10) << "t_start" << std::setw(10) << "t_end" << std::setw(14) << "H2O [mol/m³]" << std::setw(12)
            << "converged" << std::setw(10) << "steps" << std::setw(10) << "rejected" << std::setw(12) << "SO4" << std::endl;
  ...
  for (int seg = 0; seg < 10 && !failed; ++seg)
  {
    ...
    std::cout << std::fixed << std::setprecision(0) << std::setw(10) << t_start << std::setw(10) << t_end << std::scientific
              << std::setprecision(1) << std::setw(14) << h2o << std::setw(12) << (stats.converged ? "YES" : "NO")
              << std::dec << std::setw(10) << stats.total_steps << std::setw(10) << stats.total_rejected << std::scientific
              << std::setprecision(3) << std::setw(12) << state.variables_[0][i_so4mm] << std::endl;

    if (!stats.converged)
    {
      std::cout << "  *** Failed at H2O=" << h2o << ": " << SolverStateToString(stats.last_failure_state) << std::endl;
      failed = true;
    }
...
TEST(SolventRobustness, B3_CloudFormation)
{
  ...
  std::cout << "  converged=" << (stats1.converged ? "YES" : "NO") << " steps=" << stats1.total_steps
  ...
  std::cout << "  converged=" << (stats2.converged ? "YES" : "NO") << " steps=" << stats2.total_steps
            << " rejected=" << stats2.total_rejected << " SO4=" << state.variables_[0][i_so4mm] << std::endl;
...
TEST(SolventRobustness, C1_MixedColumn)
{
  ...
  std::cout << "Overall: converged=" << (stats.converged ? "YES" : "NO") << " steps=" << stats.total_steps
  ...
  // Per-cell results
  std::cout << std::setw(6) << "cell" << std::setw(14) << "H2O" << std::setw(8) << "T" << std::setw(12) << "SO4"
            << std::setw(12) << "SO2_g" << std::setw(10) << "NaN/Inf" << std::endl;
  ...
  for (std::size_t cell = 0; cell < NUM_CELLS; ++cell)
  {
    ...
    std::cout << std::setw(6) << cell << std::scientific << std::setprecision(1) << std::setw(14) << h2o_profile[cell]
              << std::fixed << std::setprecision(0) << std::setw(8) << T_profile[cell] << std::scientific
              << std::setprecision(3) << std::setw(12) << so4 << std::setw(12) << so2_g << std::setw(10)
              << (nan_inf ? "YES" : "no") << std::endl;
    ...
    {
      EXPECT_NEAR(state.variables_[cell][i_so2_aq], 0.0, 1e-15) << "Clear cell " << cell << " has non-zero SO2_aq";
      EXPECT_NEAR(state.variables_[cell][i_h2o2_aq], 0.0, 1e-15) << "Clear cell " << cell << " has non-zero H2O2_aq";
    }
  }
  std::cout << std::endl;
}
...
TEST(SolventRobustness, C2_UniformColumn)
{
  ...
  std::cout << "Converged=" << (stats.converged ? "YES" : "NO") << " steps=" << stats.total_steps
            << " rejected=" << stats.total_rejected << " f_calls=" << stats.total_function_calls << std::endl;
...
TEST(SolventRobustness, C3_ExtremeContrast)
{
  ...
  // Per-cell results
  std::cout << std::setw(6) << "cell" << std::setw(14) << "H2O" << std::setw(12) << "SO4" << std::setw(12) << "SO2_g"
            << std::setw(12) << "SO2_aq" << std::setw(10) << "NaN/Inf" << std::endl;
  ...
  for (std::size_t cell = 0; cell < NUM_CELLS; ++cell)
  {
    bool nan_inf = HasNanOrInf(state, cell);
    std::cout << std::setw(6) << cell << std::scientific << std::setprecision(1) << std::setw(14) << h2o_profile[cell]
              << std::setprecision(3) << std::setw(12) << state.variables_[cell][i_so4mm] << std::setw(12)
              << state.variables_[cell][i_so2_g] << std::setw(12) << state.variables_[cell][i_so2_aq] << std::setw(10)
              << (nan_inf ? "YES" : "no") << std::endl;
    ...
    {
      EXPECT_NEAR(state.variables_[cell][i_so2_aq], 0.0, 1e-15) << "Clear cell " << cell << " has non-zero SO2_aq";
      EXPECT_NEAR(state.variables_[cell][i_hso3m], 0.0, 1e-15) << "Clear cell " << cell << " has non-zero HSO3m";
    }

    // Cloudy cell should show sulfate production
    if (h2o_profile[cell] > 0.01 && stats.converged)
    {
      EXPECT_GT(state.variables_[cell][i_so4mm], 1.0) << "Cloudy cell " << cell << " should have SO4 increase";
    }
  }
  std::cout << std::endl;
}
