...
namespace
{
  ...
  void TestDissolvedConstraintVectorMatrix(std::size_t num_cells, double K_eq_val)
  {
    ...
    for (std::size_t c = 0; c < num_cells; ++c)
    {
      EXPECT_NEAR(state_params[c][keq_col], K_eq_val, K_eq_val * 1e-12) << "Cell " << c;
    }
    ...
    for (std::size_t c = 0; c < num_cells; ++c)
    {
      ...
      EXPECT_NEAR(residual[c][1], expected, std::max(std::abs(expected) * 1e-10, 1e-20)) << "Residual mismatch cell " << c;
    }
...
TEST(DissolvedEquilibriumConstraint, JacobianFDExtremeKeq)
{
  ...
  for (double K_eq_val : { 1.0e-10, 1.0, 1.0e10 })
  {
    ...
    DMP sv{ 1, 3, 0.0 };
    sv[0][0] = 1.0;
    sv[0][1] = K_eq_val;  // at equilibrium
