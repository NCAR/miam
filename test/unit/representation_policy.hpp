...
{
  micm::Species species_a{ "SPECIES_A", { { "molecular weight [kg mol-1]", 0.1 }, { "density [kg m-3]", 1000.0 } } };
  micm::Species species_b{ "SPECIES_B", { { "molecular weight [kg mol-1]", 0.2 }, { "density [kg m-3]", 2000.0 } } };
  micm::Species species_c{ "SPECIES_C", { { "molecular weight [kg mol-1]", 0.3 }, { "density [kg m-3]", 1500.0 } } };
...
inline void setTestVariables(micm::Matrix<double>& vars, const std::unordered_map<std::string, std::size_t>& var_indices)
{
    ...
    micm::Matrix<double>& vars)
{
  ...
  for (std::size_t k = 0; k < n_deps; ++k)
  {
    ...
    EXPECT_NEAR(partials[0][k], fd, tol) << "partial[" << k << "] (var idx " << var_idx << "): analytic=" << partials[0][k]
                                         << " fd=" << fd;
  }
}
