...
namespace
{
  ...
  {
    return std::sqrt(8.0 * micm::constants::GAS_CONSTANT * T / (std::numbers::pi * Mw));
  }
...
TEST(CondensationRate, FreeMolecularLimit)
{
  ...
  double c_bar = std::sqrt(8.0 * micm::constants::GAS_CONSTANT * T / (std::numbers::pi * gas_molecular_weight));
